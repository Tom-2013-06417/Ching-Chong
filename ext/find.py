...
class Find(QtGui.QDialog):
	...
	def find(self):
		...
		text = self.parent.tab.currentWidget().edit.toPlainText()
		...
		if self.lastMatch:
			...
			self.parent.tab.currentWidget().edit.moveCursor(QtGui.QTextCursor.End)

	def replace(self):

		# Grab the text cursor
		cursor = self.parent.tab.currentWidget().edit.textCursor()
		...
		if self.lastMatch and cursor.hasSelection():
			...
			self.parent.tab.currentWidget().edit.setTextCursor(cursor)
